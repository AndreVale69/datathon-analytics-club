...
def extract_constraints(query: str) -> QueryConstraints:
    ...
    try:
        ...
        hard_raw = _sanitize_filter_payload(
            {k: v for k, v in raw.get("hard", {}).items() if k not in _PAGINATION_FIELDS}
        )
        soft_raw = _sanitize_filter_payload(
            {k: v for k, v in raw.get("soft", {}).items() if k not in _PAGINATION_FIELDS}
        )
